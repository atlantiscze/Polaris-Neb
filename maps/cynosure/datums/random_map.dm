...
/datum/random_map/noise/sif/get_additional_spawns(var/value, var/turf/T)
	...
	*/
	...
	switch(value)
		if(1 to 2)
			if(prob(1))
				// new /obj/structure/flora/sif/eyes(T) //NEBFIX
			else if(prob(1))
				// new /obj/structure/flora/mushroom(T) //NEBFIX
		if(3 to 4)
			if(prob(1))
				// new /obj/structure/flora/sif/eyes(T) //NEBFIX
			else if(prob(1))
				// new /obj/structure/flora/sif/tendrils(T) //NEBFIX
			else if(prob(1))
				// new /obj/structure/flora/mushroom(T) //NEBFIX
		if(5 to 6)
			if(prob(5))
				// new /obj/structure/flora/tree/sif(T) //NEBFIX
			else if(prob(5))
				// new /obj/structure/flora/sif/tendrils(T) //NEBFIX
			else if (prob(5))
				// new /obj/structure/flora/sif/eyes(T) //NEBFIX
			else if(prob(1))
				// new /obj/structure/flora/sif/frostbelle(T) //NEBFIX
		if(7 to 8)
			if(prob(5))
				// new /obj/structure/flora/tree/sif(T) //NEBFIX
			else if(prob(5))
...
/datum/random_map/noise/sif/underground
	...
	// skip_dense = TRUE //NEBFIX
	// keep_outside = TRUE //NEBFIX
...
/datum/random_map/noise/sif/underground/get_additional_spawns(var/value, var/turf/T)
	if(value <= 1 && prob(30)) // Mud is very fun-gy.
		...
		if(mushroom_prob && prob(mushroom_prob))
			...
			// new /obj/structure/flora/sif/subterranean(T) //NEBFIX
*/
