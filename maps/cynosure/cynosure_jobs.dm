...
/decl/department/planetside
	name = "Planet"
	// color = "#555555" //NEBFIX
	// sorting_order = 2 // Same as cargo in importance. //NEBFIX

/datum/job/explorer
	title = "Explorer"
	// flag = EXPLORER //NEBFIX
	// departments = list(DEPARTMENT_RESEARCH, DEPARTMENT_PLANET) //NEBFIX
	// department_flag = MEDSCI //NEBFIX
	// faction = "Station" //NEBFIX
	total_positions = 0 //NEBFIX - Used to be "4" - Disabled due to large amount of nebula switch related bugs.
	spawn_positions = 0
	supervisors = "the Research Director"
	selection_color =  "#633D63"
	// economic_modifier = 4 //NEBFIX
	access = list(access_explorer, access_research)
	minimal_access = list(access_explorer, access_research)

	// outfit_type = /decl/hierarchy/outfit/job/explorer2 //NEBFIX
	// job_description = "An Explorer searches for interesting things on the surface of Sif, and returns them to the station." //NEBFIX

	// alt_titles = list("Pilot" = /datum/alt_title/pilot) //NEBFIX
...
/datum/job/paramedic
	...
	alt_titles = list(
					...
					"Search and Rescue" = /datum/alt_title/sar) //NEBFIX
	*/
...
/datum/job/survivalist
	title = "Survivalist"
	// departments = list(DEPARTMENT_PLANET) //NEBFIX
	// department_flag = CIVILIAN //NEBFIX
	// flag = HERMIT //NEBFIX
	selection_color = "#6085a8"
	total_positions = 0 //NEBFIX - Disabled temporarily. Was "3"
	spawn_positions = 0
	// faction = "Station" //NEBFIX
	supervisors = "your conscience"
	// economic_modifier = 1 //NEBFIX
	access = list()
	minimal_access = list()
	// outfit_type = /decl/hierarchy/outfit/job/survivalist //NEBFIX
	// job_description = "There are a few small groups of people living in the wilderness of Sif, and they occasionally venture to the Cynosure to trade, ask for help, or just have someone to talk to."//NEBFIX
	// assignable = FALSE //NEBFIX
	// has_headset = FALSE //NEBFIX
	account_allowed = FALSE
	// offmap_spawn = TRUE //NEBFIX
	// announce_arrival_and_despawn = FALSE //NEBFIX
	// substitute_announce_title = "Colonist" //NEBFIX
	// banned_job_species = null //NEBFIX
	// alt_titles = list(CRASH_SURVIVOR_TITLE = /datum/alt_title/crash_survivor) //NEBFIX
	var/const/SPAWN_DAMAGE_MULT = 0.15
	var/const/TOTAL_INJURIES = 3
	var/static/list/pod_templates = list(
		// /datum/map_template/surface/plains/Epod, //NEBFIX
		// /datum/map_template/surface/plains/Epod2, //NEBFIX
		///datum/map_template/surface/wilderness/deep/Epod3, // Don't use this one, it has spiders.
		// /datum/map_template/surface/wilderness/normal/Epod4  //NEBFIX
	)
...
/* //NEBFIX
/datum/job/survivalist/equip(mob/living/human/H, alt_title)
	// . = ..()
	if(.)
...
/obj/item/card/id/drake_expedition
	...
	// registered_id_string = "Registration Card" //NEBFIX
	access = list(
		access_explorer,
		access_research,
		access_xenofauna
...
/obj/item/flashlight/glowstick/grafadreka/Initialize()
	...
	// flashlight_colour = initial(archetype.flashlight_colour) //NEBFIX
	icon_state = initial(archetype.icon_state)
	item_state = initial(archetype.item_state)
	// fuel = rand(3200, 4800) //NEBFIX
	on = TRUE
	update_icon()
